...
describe("Accordion.cy.jsx", () => {
  it("Items accordion", () => {
    ...
    cy.getDataTest("accordion-wrapper").should("exist");
    ...
    const detailsRegex = new RegExp(
      items[0].details.replace(/\s+/g, "\\s*"),
      "g",
    );
    cy.contains(detailsRegex).should("not.be.visible");
    cy.getDataTest("accordion-item-1").click();
    cy.contains(detailsRegex).should("be.visible");
